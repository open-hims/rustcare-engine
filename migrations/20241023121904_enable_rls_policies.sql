...
BEGIN
    ...
    ELSIF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'auth_audit_log') THEN
...
-- Roles: Access global roles (NULL) OR organization-specific roles (if table exists)
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'roles') THEN
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'audit_logs') THEN
        EXECUTE 'CREATE POLICY audit_logs_org_isolation ON audit_logs
            FOR SELECT
            USING (organization_id = current_setting(''app.organization_id'', true)::UUID)';
        
        EXECUTE 'CREATE POLICY audit_logs_system_insert ON audit_logs
            FOR INSERT
            WITH CHECK (organization_id = current_setting(''app.organization_id'', true)::UUID)';
    ELSIF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'auth_audit_log') THEN
        EXECUTE 'CREATE POLICY auth_audit_log_org_isolation ON auth_audit_log
            FOR SELECT
            USING (organization_id = current_setting(''app.organization_id'', true)::UUID)';
        
        EXECUTE 'CREATE POLICY auth_audit_log_system_insert ON auth_audit_log
            FOR INSERT
            WITH CHECK (organization_id = current_setting(''app.organization_id'', true)::UUID)';
    END IF;
END $$;
