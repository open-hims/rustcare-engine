...
use crate::db::{DatabasePool, QueryExecutor, RlsContext};
...
impl OrganizationRepository {
        ...
        if let Some(logger) = &self.audit_logger {
            let _ = logger
                .log_event(
                    ...
                    "organization",
                    organization_id,
                    None, // No specific user for org operations
                    details,
                )
                .await;
        }
    ...
    pub async fn create(&self, org: CreateOrganization) -> Result<Organization> {
        let query_executor = QueryExecutor::new(
            self.pool.clone(),
            self.rls_context.clone(),
            self.audit_logger.clone(),
        );

        let organization = query_executor
            .execute(|conn| async move {
                ...
                .bind(org.id.unwrap_or_else(Uuid::new_v4))
                .bind(&org.name)
                .bind(&org.slug)
                .bind(&org.domain)
    ...
    pub async fn find_by_domain(&self, domain: &str) -> Result<Option<Organization>> {
        let query_executor = QueryExecutor::new(
            self.pool.clone(),
            self.rls_context.clone(),
            self.audit_logger.clone(),
        ...
        let organization = query_executor
            ...
            .await
            .context("Failed to find organization by domain")?;
    ...
    pub async fn list(&self, limit: i64, offset: i64) -> Result<Vec<Organization>> {
        ...
        let organizations = query_executor
            .execute(|conn| async move {
                sqlx::query_as::<_, Organization>(
                    r#"
                    SELECT * FROM organizations
    ...
    pub async fn list_active(&self, limit: i64, offset: i64) -> Result<Vec<Organization>> {
        let query_executor = QueryExecutor::new(
            self.pool.clone(),
            self.rls_context.clone(),
            self.audit_logger.clone(),
        ...
        let organizations = query_executor
            ...
            .await
            .context("Failed to list active organizations")?;

        Ok(organizations)
    }

    /// Update organization
    pub async fn update(&self, id: Uuid, update: UpdateOrganization) -> Result<Organization> {
        let query_executor = QueryExecutor::new(
            self.pool.clone(),
            self.rls_context.clone(),
            self.audit_logger.clone(),
    ...
    pub async fn verify_organization(&self, id: Uuid) -> Result<Organization> {
        let query_executor = QueryExecutor::new(
            self.pool.clone(),
            self.rls_context.clone(),
            self.audit_logger.clone(),
        ...
        let organization = query_executor
            ...
            .await
            .context("Failed to verify organization")?;
    ...
    pub async fn soft_delete(&self, id: Uuid) -> Result<()> {
        ...
        query_executor
            ...
            .await
            .context("Failed to soft delete organization")?;
    ...
    pub async fn restore(&self, id: Uuid) -> Result<Organization> {
        let query_executor = QueryExecutor::new(
            self.pool.clone(),
            self.rls_context.clone(),
            self.audit_logger.clone(),
    ...
    pub async fn get_user_count(&self, organization_id: Uuid) -> Result<i64> {
        let query_executor = QueryExecutor::new(
            self.pool.clone(),
            self.rls_context.clone(),
            self.audit_logger.clone(),
    ...
    pub async fn get_storage_usage(&self, organization_id: Uuid) -> Result<i64> {
    ...
    pub async fn has_reached_user_limit(&self, organization_id: Uuid) -> Result<bool> {
        let org = self
    ...
    pub async fn has_reached_storage_limit(&self, organization_id: Uuid) -> Result<bool> {
        let org = self
            .find_by_id(organization_id)
            .await?
            .ok_or_else(|| anyhow::anyhow!("Organization not found"))?;
    ...
    pub async fn get_system_organization(&self) -> Result<Option<Organization>> {
